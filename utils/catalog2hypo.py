        ...
        for event in catalog:
            ...
            for pick in event.picks:
                ...
                if station not in picks:
                    picks[station] = {"P":{"phase":" ", "time":"               ", "w":" "}, "S":{"phase":" ", "time":"      ", "w":" "}}
            ...
            for station in picks:
                ...
                if picks[station]["S"]["phase"].strip():
                    ...
                    arrivaltimeS = picks[station]["S"]["time"] - picks[station]["P"]["time"].replace(second=0, microsecond=0)
                    arrivaltimeS = "{arrivaltimeS:6.2f}".format(arrivaltimeS=arrivaltimeS)
...
def velocity2hypoellipse(velocityDict, outFile):
    with open(outFile, "w") as f:
        for v,z in zip(velocityDict["Vp"], velocityDict["Z"]):
            f.write("VELOCITY             {Vp:4.2f} {Z:5.2f} {VpVs:4.2f}\n".format(Vp=v, Z=z, VpVs=velocityDict["VpVs"]))
            # f.write("VELOCITY             %4.2f %5.2f %4.2f\n"%(v, z, sta0["CNTL"]["VpVs"]))

def generateHypoellipseDefaultFile(defaultsDict, outFile):
    with open(outFile, "w") as f:
        f.write("reset test         1    {0:6.2f}\n".format(defaultsDict["VpVs"]))
        f.write("reset test         2    {0:6.2f}\n".format(defaultsDict["elevationCorrection"][0]))
        f.write("reset test         8    {0:6.2f}\n".format(defaultsDict["elevationCorrection"][1]))
        f.write("reset test         5    {0:6.2f}\n".format(defaultsDict["startingDepth"]))
        f.write("reset test         10   {0:6.2f}\n".format(defaultsDict["distanceWeighting"][0]))
        f.write("reset test         11   {0:6.2f}\n".format(defaultsDict["distanceWeighting"][1]))
        f.write("reset test         12   {0:6.2f}\n".format(defaultsDict["distanceWeighting"][2]))
        f.write("reset test         21   {0:6.2f}\n".format(defaultsDict["maximumNumberOfIterations"]))
        f.write("reset test         29   {0:6.2f}\n".format(defaultsDict["standardErrorForArrivalTimesWithWeightCode0"]))
        f.write("reset test         38   {0:6.2f}\n".format(defaultsDict["locateWithS"]))
        f.write("reset test         39   {0:6.2f}\n".format(defaultsDict["factorForWeightsOfSAndS_PTimes"]))
        f.write("summary option     {0:1.0f}\n".format(defaultsDict["summaryOption"]))
        f.write("printer option     {0:1.0f}\n".format(defaultsDict["printerOption"]))
        f.write("constants noprint  {0:1.0f}\n".format(defaultsDict["constantsNoPrint"]))
        f.write("compress option    {0:1.0f}\n".format(defaultsDict["compressOption"]))
        f.write("tabulation option  {0:1.0f}\n".format(defaultsDict["tabulationOption"]))
        f.write("weight option      {0:4.2f} {0:4.2f} {0:4.2f}\n".format(defaultsDict["weightOption"][0], defaultsDict["weightOption"][1], defaultsDict["weightOption"][2]))
        f.write("ignore summary rec {0:1.0f}\n".format(defaultsDict["ignoreSummaryRec"]))
        f.write("header option      {0:s}\n".format(defaultsDict["headerOption"]))
